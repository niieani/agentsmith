...
An issue-tracker pack supplies a reusable `tracker` skill-slot snippet. Every enabled skill that declares that slot receives the selected tracker instructions, so the pack does not need to know which skills consume it. A complex grilling skill includes shared private partials at generation time. Emitted skills are flat and self-contained; the harness does not need to invoke or load implementation-building-block skills at runtime.
...
- Multiple packs may contribute to one enabled skill, and one pack skill-slot contribution may feed multiple enabled skills.
...
13. One selected project-owned tracker snippet can fill the matching slot in multiple shared skills without naming those skills in the pack source tree.
